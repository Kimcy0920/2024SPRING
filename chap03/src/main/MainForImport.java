...
public class MainForImport { // 객체 생성을 하지 않고 사용하기 위함

	private static ApplicationContext ctx = null; // 수정1, 정적으로 선언해 객체 생성 없이 사용할 수 있게함.
	
	public static void main(String[] args) throws IOException {
		
		ctx = new AnnotationConfigApplicationContext(AppConfImport.class); // 수정2
...
//		VersionPrinter versionPrinter = ctx.getBean(VersionPrinter.class); // "versionPrinter" getBean 이름이 하나만 있는 경우에 생략 가능
		VersionPrinter versionPrinter = ctx.getBean("versionPrinter", VersionPrinter.class); // 같은 이름이 또 있으면 안됨 
